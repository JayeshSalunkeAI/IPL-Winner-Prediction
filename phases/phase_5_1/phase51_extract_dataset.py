...
def _lineup_set(players: list[str]) -> set[str]:
    return {p for p in players if p != "Unknown_Player"}
...
def _venue_phase_prior(values: list[float], default: float) -> float:
    if not values:
        return default
    return float(np.mean(values))


def build_phase51_dataset(yaml_dir: Path, output_csv: Path, player_slots: int = 11) -> pd.DataFrame:
    ...
    venue_toss_bat_count: dict[str, int] = defaultdict(int)
    venue_toss_count: dict[str, int] = defaultdict(int)
    venue_toss_team1_decision_games: dict[tuple[str, int, str], int] = defaultdict(int)
    venue_toss_team1_decision_wins: dict[tuple[str, int, str], int] = defaultdict(int)

    venue_pp_rr_first_hist: dict[str, list[float]] = defaultdict(list)
    venue_death_rr_first_hist: dict[str, list[float]] = defaultdict(list)
    venue_pp_rr_second_hist: dict[str, list[float]] = defaultdict(list)
    venue_death_rr_second_hist: dict[str, list[float]] = defaultdict(list)

    last_lineup_by_team: dict[str, set[str]] = {}
    player_last_seen_date: dict[str, pd.Timestamp] = {}
    ...
    for path in yaml_paths:
        ...
        p1_set = _lineup_set(p1)
        p2_set = _lineup_set(p2)
        ...
        # New: recent XI continuity prior.
        prev_t1 = last_lineup_by_team.get(team1, set())
        prev_t2 = last_lineup_by_team.get(team2, set())
        t1_xi_cont = len(p1_set & prev_t1) / max(len(p1_set), 1) if prev_t1 else 0.5
        t2_xi_cont = len(p2_set & prev_t2) / max(len(p2_set), 1) if prev_t2 else 0.5

        # New: player availability/injury proxy via recency of player appearances.
        lookback_days = 450
        t1_active = 0
        for p in p1_known:
            last_seen = player_last_seen_date.get(p)
            if last_seen is not None and (date - last_seen).days <= lookback_days:
                t1_active += 1
        t2_active = 0
        for p in p2_known:
            last_seen = player_last_seen_date.get(p)
        ...
        t1_availability = t1_active / max(len(p1_known), 1) if p1_known else 0.5
        t2_availability = t2_active / max(len(p2_known), 1) if p2_known else 0.5
        t1_injury_proxy = 1.0 - t1_availability
        t2_injury_proxy = 1.0 - t2_availability

        # New: toss-decision x venue interaction priors.
        toss_team1 = 1 if toss_winner == team1 else 0
        venue_toss_bat_rate = venue_toss_bat_count[venue] / venue_toss_count[venue] if venue_toss_count[venue] > 0 else 0.5
        toss_decision_alignment = 0.5
        if toss_decision in {"bat", "field"}:
            toss_decision_alignment = 1.0 if ((venue_toss_bat_rate >= 0.5 and toss_decision == "bat") or (venue_toss_bat_rate < 0.5 and toss_decision == "field")) else 0.0

        td_key = (venue, toss_team1, toss_decision)
        td_games = venue_toss_team1_decision_games[td_key]
        td_team1_wins = venue_toss_team1_decision_wins[td_key]
        toss_decision_venue_team1_winrate = (td_team1_wins + 1) / (td_games + 2)

        toss_field_chase_boost_team1 = 0.0
        if toss_team1 == 1 and toss_decision == "field":
            toss_field_chase_boost_team1 = venue_chase_prior
        elif toss_team1 == 0 and toss_decision == "field":
            toss_field_chase_boost_team1 = 1.0 - venue_chase_prior

        # New: venue phase split priors.
        venue_pp_rr_first_prior = _venue_phase_prior(venue_pp_rr_first_hist[venue], default=7.5)
        venue_death_rr_first_prior = _venue_phase_prior(venue_death_rr_first_hist[venue], default=9.5)
        venue_pp_rr_second_prior = _venue_phase_prior(venue_pp_rr_second_hist[venue], default=7.3)
        venue_death_rr_second_prior = _venue_phase_prior(venue_death_rr_second_hist[venue], default=9.2)
        ...
        record: dict[str, Any] = {
            ...
            "team1_player_availability_prior": t1_availability,
            "team2_player_availability_prior": t2_availability,
            "team1_injury_proxy_prior": t1_injury_proxy,
            "team2_injury_proxy_prior": t2_injury_proxy,
            "venue_toss_bat_rate_prior": venue_toss_bat_rate,
            "toss_decision_alignment_prior": toss_decision_alignment,
            "toss_decision_venue_team1_winrate_prior": toss_decision_venue_team1_winrate,
            "toss_field_chase_boost_team1": toss_field_chase_boost_team1,
            "venue_pp_rr_first_innings_prior": venue_pp_rr_first_prior,
            "venue_death_rr_first_innings_prior": venue_death_rr_first_prior,
            "venue_pp_rr_second_innings_prior": venue_pp_rr_second_prior,
            "venue_death_rr_second_innings_prior": venue_death_rr_second_prior,
            "Match_Winner": winner,
        }
        ...
        h2h_games[(team1, team2)] += 1
        if winner == team1:
            h2h_team1_wins[(team1, team2)] += 1
        ...
        if toss_decision in {"bat", "field"}:
            venue_toss_count[venue] += 1
            if toss_decision == "bat":
                venue_toss_bat_count[venue] += 1

        if toss_winner in {team1, team2}:
            td_key = (venue, 1 if toss_winner == team1 else 0, toss_decision)
            venue_toss_team1_decision_games[td_key] += 1
            if winner == team1:
                venue_toss_team1_decision_wins[td_key] += 1
        ...
        if len(batting_order) >= 2:
            ...
            first_balls = max(_to_float((stats.get(bat_first) or {}).get("balls", 0.0), 0.0), 1.0)
            second_balls = max(_to_float((stats.get(bat_second) or {}).get("balls", 0.0), 0.0), 1.0)

            first_pp_rr = 6.0 * _to_float((stats.get(bat_first) or {}).get("powerplay_runs", 0.0), 0.0) / min(first_balls, 36.0)
            first_death_rr = 6.0 * _to_float((stats.get(bat_first) or {}).get("death_runs", 0.0), 0.0) / max(first_balls - 96.0, 1.0)
            second_pp_rr = 6.0 * _to_float((stats.get(bat_second) or {}).get("powerplay_runs", 0.0), 0.0) / min(second_balls, 36.0)
            second_death_rr = 6.0 * _to_float((stats.get(bat_second) or {}).get("death_runs", 0.0), 0.0) / max(second_balls - 96.0, 1.0)

            venue_pp_rr_first_hist[venue].append(first_pp_rr)
            venue_death_rr_first_hist[venue].append(first_death_rr)
            venue_pp_rr_second_hist[venue].append(second_pp_rr)
            venue_death_rr_second_hist[venue].append(second_death_rr)
        ...
        for p in p1_known:
            ...
            player_last_seen_date[p] = date
        for p in p2_known:
            player_elo[p] = player_elo.get(p, base_elo) + d2
            player_last_seen_date[p] = date

        last_lineup_by_team[team1] = p1_set
        last_lineup_by_team[team2] = p2_set
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Phase 5.1 dataset with recency-weighted and matchup interaction priors.")
    parser.add_argument("--yaml-dir", default=str(ROOT_DIR / "data/raw/ipl_male"))
    parser.add_argument("--output", default=str(ROOT_DIR / "phases/phase_5_1/results/phase51_dataset.csv"))
...
def main() -> None:
    args = parse_args()
    data = build_phase51_dataset(
        yaml_dir=Path(args.yaml_dir),
        output_csv=Path(args.output),
        player_slots=args.player_slots,
    )
    print(f"Saved Phase 5.1 dataset: {args.output} ({len(data)} rows)")
